...
private:
	...
	void inline SDUmbraFair(double* oldver, double* newver, double* oldfirstdiffer, double* newfirstdiffer, 
		double* seconddiffer, vector<int>&vermark, vector<intvector>& verneighbr,HashMap& ver2edgehash, vector<int>&edgelist, vector<double>&wlist);
	void SDUmbraFair(int times);
...
public:
	void InputData( const std::vector<float> &mver, const intvector &mface, const intvector &ctrmedge, const bool &doswap);
    void InputData( const std::vector<float> &mver,const intvector &mface, const intvector &mfaceMat,const intvector &ctrmedge, const bool &doswap);
	void OutputData( std::vector<double> &mver, intvector &mface);
	void OutputData( std::vector<float> &mver, intvector &mface);
    void OutputData( std::vector<float> &mver, intvector &mface,intvector &mfaceMat);
