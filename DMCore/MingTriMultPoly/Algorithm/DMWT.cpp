...
int DMWT::newTile(__int64_t wec, float c, int t, int nxt1, int nxt2){
...
inline void DMWT::appendTile(__int64_t wec, float c, int t, int nxt1, int nxt2, vector<OneTile> &locTileVec){
...
inline int DMWT::appendTile(__int64_t wec, float c, int t, int nxt1, int nxt2, vector<int> &curTile){
...
int DMWT::computeTriangulation(Boundary<MAXK>& B, Hole& H){
	...
	__int64_t toAvoidK1,toAvoidK2, toAvoidK;
					...
					for (int h=0; h<numSubHHs; h++){
						...
						for (int tl1=0; tl1<subRes1End-subRes1Start+1; tl1++){
							for (int tl2=0; tl2<subRes2End-subRes2Start+1; tl2++){
								...
								__int64_t WEK1 = allOneTiles[oti1].WECode;
								float C1   = allOneTiles[oti1].cost;
								__int64_t WEK2 = allOneTiles[oti2].WECode;
			...
			case 2:
				...
				for (int b=0; b<numSubBBs; b++){
					...
					for (int tl=0; tl<subResEnd-subResStart+1; tl++){
						...
						__int64_t WWEEK = allOneTiles[oti].WECode;
...
__int64_t DMWT::GetBitKey(const vector<int>& canItem,const vector<int>& allItem){
	__int64_t key = 0, mask = 1, one = 1;
...
void DMWT::RecoverListFromBitKey(const vector<int>& list, __int64_t key,vector<int>& sublist ){
	sublist.clear();
	int size = (int)list.size();
	__int64_t mask = 1, exist;
	int item, pos = 0;
...
// v1<v2
inline bool DMWT::isBDEdge(int v1, int v2){
...
//FOR LIFAN
void DMWT::loadCurves(vector<vector<vector<float> > > &inCurves){
...
void DMWT::loadNormals(vector<vector<vector<float> > > &inNorms){
...
void DMWT::saveTiling(){
	...
	strncpy(tilefile,filename,300);
	if (useDT) {
		strncat(tilefile,"_dyn",300);
	} else {
        strncat(tilefile,"_all",300);
	}
	if (saveObj){
        strncat(tilefile,".obj",300);
...
void DMWT::saveResults(vector<vector<int> > & sufF){
	int t = 0;
	int n = (int) optTile.size();
	vector<int> oneF(3);
	sufF.reserve(n);
...
void DMWT::saveResults(
	...
	_pPositions = new float [_numofpoints*3];
	_pNormals = new float [_numofpoints*3];
	_pFaceIndices = new int [_numoftilingtris*3];

	// write vertices & normals
	for(int i=0; i<_numofpoints; i++){
		_pPositions[i*3] = vertex(i,0);
		_pPositions[i*3+1] = vertex(i,1);
		_pPositions[i*3+2] = vertex(i,2);

		if(useNormal){
			_pNormals[i*3] = normal(i,0);
			_pNormals[i*3+1] = normal(i,1);
			_pNormals[i*3+2] = normal(i,2);
		}
	...
	for (int i=0; i<_numoftilingtris; i++){
		t = optTile[i];
		_pFaceIndices[i*3] = triangle(t,0);
		_pFaceIndices[i*3+1] = triangle(t,1);
		_pFaceIndices[i*3+2] = triangle(t,2);
...
inline float DMWT::costTri(float measure){
	return weightTri*measure;
}

inline float DMWT::costEdge(float measure){
	return weightEdge*measure;
}

inline float DMWT::costBiTri(float measure){
	return weightBiTri*measure*measure;
}

inline float DMWT::costTriBd(float measure){
	return weightTriBd*measure*measure;
}

inline float DMWT::measureEdge(int v1, int v2){
	Point3 p1 = point(v1); Point3 p2 = point(v2);
	return (p2-p1).length();
}

inline float DMWT::measureTri(int v1, int v2, int v3){
	Point3 p1 = point(v1); Point3 p2 = point(v2); 
	Point3 p3 = point(v3);
	return ((p2-p1)^(p3-p2)).length()/2;
}

inline float DMWT::measureBiTri(int v1, int v2, int p, int q){
	Point3 p1 = point(v1); Point3 p2 = point(v2); 
...
inline float DMWT::CostBD(int e, int i){
	if (useNormal)
		return weightBiTri*edgeInfoList[e]->bdMeasure[i];
	return 0.0;
}

inline int DMWT::getBiPos(int len, int i, int j){
	return (int)((i+1) * (2*len-(i+1)-1) / 2 + (j+1) - len - 1);
}

//==================================Help Func============================//
/// Check whether boundary B is a board edge
inline bool DMWT::isBoardEdge(const Boundary<MAXK>& B){
...
inline bool DMWT::isMeetingSpanE(const Boundary<MAXK>& B){
...
inline void DMWT::getSortV(int e, int s, int& v1, int& v2){
	if (s == 1) {
...
inline int DMWT::nextV(int v, int dir){
...
inline void DMWT::getSubActiveEdge(int e, int ff, int v1, int v2, int& ee1, int& ee2, short&ffi1, short&ffi2){
	int e1, e2, fi1, fi2;
	e1 = EinT(ff,0);
	fi1 = TIinT(ff,0);
	e2  = EinT(ff,1);
...
inline bool DMWT::isWithinInterval(int pv, int nv, int v, int d){
...
inline void DMWT::getIntervalsB(const Boundary<MAXK>& B, Interval<MAXK>& Intvs){
	if (B.getSize() <= 0) {
		return;
	}
...
inline float DMWT::findMax(float val1, float val2, float val3){
	if (val1 > val2){
		if (val1 > val3)
			return val1;
		return val3;
...
inline float DMWT::findMax(float val1, float val2, float val3, float val4){
...
inline bool DMWT::isInvalidB(Boundary<MAXK>& B){
...
bool DMWT::EdgeProtection(){
	...
	if(!isProtected()){
		if (!badInput){
			cout << "New vertices are added to the input curve..." << endl;
			protectCorner();
